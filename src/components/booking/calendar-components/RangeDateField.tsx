...
export const RangeDateField = ({
  ...
  if (!state) {
    throw new Error(
      "<RangeDateField /> must be used within a <RangeCalendar /> compoment",
    );
  }
  ...
  const handleChange = (date: DateValue | null) => {
    ...
    if (part === "start") {
      const end =
        current?.end && current.end.compare(date) >= 0 ? current.end : date;
      state.setValue({ start: date, end });
    } else {
      const start =
        current?.start && current.start.compare(date) <= 0
          ? current.start
          : date;
